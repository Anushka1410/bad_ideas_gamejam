...
def draw_box_clue():
    ...
    clue2 = big_font.render("Manifest WIN", True, (255, 120, 120))
    screen.blit(clue1, clue1.get_rect(center=(LENGTH // 2, 80)))
    screen.blit(clue2, clue2.get_rect(center=(LENGTH // 2, 120)))
...
def draw_backup_hint():
    if show_backup_hint:
        ...
        screen.blit(hint, hint.get_rect(center=(LENGTH // 2, 200)))
...
def draw_typed_code():
    if box_opened and not level_cleared:
        code_text = small_font.render(f"Typed: {typed_code}", True, (255, 255, 255))
...
while gameloop == True:
    ...
    if menu_state == "pause":
        ...
        if level_cleared:
            etitle = font.render("YOU WIN. CLEARLY.", True, (0, 255, 100))
        else:
